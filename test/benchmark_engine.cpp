...
BenchResult RunBenchmark(AstrometryEngine& engine, ResultBuffer& buffer,
                         size_t count, const Observer& obs,
  ...
  return BenchResult{
      .count = count,
      .set_catalog_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                            end_set - start_set)
                            .count(),
      .calculate_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                          end_calc - start_calc)
                          .count(),
...
TEST_CASE("Engine Performance Benchmarking", "[.benchmark]") {
  ...
  SECTION("Scaling Tests") {
    ...
    std::cout << std::left << std::setw(10) << "Stars" << std::setw(15)
              << "Set (ms)" << std::setw(15) << "Calc (ms)" << std::setw(12)
              << "Results" << std::setw(12) << "Allocs" << std::setw(12)
              << "Memory (KB)" << "\n";
    std::cout << std::string(80, '-') << "\n";

    for (size_t size : sizes) {
      auto res = RunBenchmark(engine, buffer, size, obs, now);
      std::cout << std::left << std::setw(10) << res.count << std::setw(15)
                << res.set_catalog_ms << std::setw(15) << res.calculate_ms
                << std::setw(12) << res.result_count << std::setw(12)
                << res.allocations << std::setw(12) << res.allocated_kb << "\n";
    }
    std::cout << "\n* Note: 'Memory' includes internal parallel intermediate "
                 "buffers.\n";
    std::cout << std::string(80, '=') << "\n" << std::endl;
  }
}
